"""Database helpers and paper-supply catalog."""
import ast
...
# ---------------------------------------------------------------------------
# Engine (module-level singleton, set by init_database)
# ---------------------------------------------------------------------------

db_engine: Engine = create_engine("sqlite:///munder_difflin.db")
...
def generate_sample_inventory(supplies: list, coverage: float = 0.4, seed: int = 137) -> pd.DataFrame:
    ...
    selected_items = [supplies[i] for i in selected_indices]
    inventory = []
...
def init_database(engine: Engine, seed: int = 137) -> Engine:
    ...
    try:
        ...
        quotes_df = quotes_df[["request_id", "total_amount", "quote_explanation", "order_date",
                               "job_type", "order_size", "event_type"]]
        quotes_df.to_sql("quotes", engine, if_exists="replace", index=False)

        inventory_df = generate_sample_inventory(paper_supplies, seed=seed)
        initial_transactions = [{"item_name": None, "transaction_type": "sales",
...
def generate_financial_report(as_of_date: Union[str, datetime]) -> Dict:
    ...
    for _, item in inventory_df.iterrows():
        ...
        inventory_summary.append({"item_name": item["item_name"], "stock": stock,
...
def search_quote_history(search_terms: List[str], limit: int = 5) -> List[Dict]:
    ...
    for i, term in enumerate(search_terms):
        p = f"term_{i}"
        conditions.append(f"(LOWER(qr.response) LIKE :{p} OR LOWER(q.quote_explanation) LIKE :{p})")
